...
results_template = "unif_bound_eta_{eta}_N_{N_thetas}.csv"
...
def estimate_R_eps(eps, eta, N_theta,N_zetas):
    ...
    #z_i~Pi
    zetas = np.random.uniform(A, B, size=N_zetas) #In this case unif
...
def sweep_eps(eps_values, eta, N_theta,N_zetas, M, n_jobs=n_jobs):
    R_finals = []
    CI_lower = []
    CI_upper = []
...
if __name__ == "__main__":
    ...
    eps_values = np.linspace(0.1, 10.0, 6)
    ...
    for eta, color,color2 in zip(eta_values, colors,colors2):
        ...
        for epsilon in eps_values:
            scale=(B-A)/epsilon
            bound = 1-(1-eta)*np.exp(-epsilon*eta)
            new =  2*eta-eta**2+(scale**2)*(np.exp((eta-1)/scale)+np.exp(-eta/scale)-np.exp(-1/scale)-1)
            uniform_bound.append((bound-new))
        """
        # ------------------------------------------------
        # PLOT
        # ------------------------------------------------
    ...
    plt.tight_layout()
    plt.savefig(f"unif_{N_thetas}.png", dpi=300)
